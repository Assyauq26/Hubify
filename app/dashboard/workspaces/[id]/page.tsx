  ...
  return (
    <div className="min-w-0 bg-[var(--background)] px-[var(--content-gutter)] py-6 text-[var(--foreground)] sm:py-8 lg:px-10 lg:py-10">
      <div className="mx-auto max-w-6xl">
        ...
        <header className="mt-3 border-b border-[var(--border)] pb-6 sm:pb-7">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[var(--muted-foreground)]">Workspace</p>
              <h1 className="mt-2 break-words text-[28px] font-semibold leading-tight tracking-[-0.03em] sm:text-[32px]">{workspace.name}</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--muted-foreground)]">{workspace.description || 'Keep the resources for this work together.'}</p>
            </div>
            <Link href={`/dashboard/workspaces/${id}/resources/new`} className="ui-button-primary min-h-11 shrink-0">+ Add resource</Link>
          </div>
        </header>
