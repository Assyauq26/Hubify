...
export default function WorkspaceTabs({ workspaceId, resources }: Props) {
  ...
  const setDirty = (resourceId: string, dirty: boolean) => {
    setTabs((current) => current.map((tab) => tab.resourceId === resourceId ? { ...tab, dirty } : tab))
  }

  const updateTitle = (resourceId: string, title: string) => {
    setTabs((current) => current.map((tab) => tab.resourceId === resourceId ? { ...tab, title: title || 'Untitled note' } : tab))
  }
    ...
    <section aria-label="Workspace tabs" className="mt-6">
      <div role="tablist" aria-label="Open resources" className="flex min-h-11 items-stretch overflow-x-auto border-y border-[var(--border)] bg-[var(--surface)]">
        {tabs.map((tab) => {
          ...
          return <div key={tab.resourceId} className={`flex shrink-0 items-center border-r border-[var(--border)] ${active ? 'bg-[var(--background)]' : 'bg-[var(--surface)]'}`}>
            <button type="button" role="tab" aria-selected={active} aria-controls={`workspace-panel-${tab.resourceId}`} onClick={() => void activate(tab.resourceId)} className={`min-h-11 max-w-48 truncate px-4 text-left text-xs font-medium sm:max-w-64 ${active ? 'text-[var(--foreground)]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'}`}>
      ...
      <div className="mt-4 border-b border-[var(--border)] pb-6">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {resources.map((resource) => <button key={resource.id} type="button" onClick={() => void open(resource)} className="border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs text-[var(--foreground)] hover:bg-[var(--surface-secondary)]">{resource.title}</button>)}
          {resources.length === 0 ? <p className="text-sm text-[var(--muted-foreground)]">Add a resource to open it as a tab.</p> : null}
        </div>
      </div>

      {activeId ? (
        <div id={`workspace-panel-${activeId}`} role="tabpanel" aria-label={resourceMap.get(activeId)?.title ?? 'Active resource'} className="mt-6 overflow-hidden border border-[var(--border)] bg-[var(--surface)]">
          {resourceMap.get(activeId)?.type === 'note' ? (
            <WorkspaceNoteEditor
              resourceId={activeId}
              workspaceId={workspaceId}
              onDirtyChange={(dirty) => setDirty(activeId, dirty)}
              onTitleChange={(title) => updateTitle(activeId, title)}
            />
          ) : (
            <div className="p-8">
              ...
              <a href={`/dashboard/resources/${activeId}`} className="mt-3 inline-flex text-sm font-medium underline underline-offset-2">Open resource viewer</a>
            </div>
          )}
        </div>
      ) : (
        <div className="mt-6 border border-dashed border-[var(--border-strong)] p-8 text-center"><p className="text-sm font-medium">No resource is open</p><p className="mt-1 text-sm text-[var(--muted-foreground)]">Select a resource below to open it as an internal tab.</p></div>
      )}
    </section>
  )
}
