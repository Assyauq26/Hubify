...
export default function WorkspaceTabs({ workspaceId, resources }: Props) {
  ...
  useEffect(() => {
    ...
    const validTabs = stored.tabs
      ...
      .map((tab) => ({
        ...tab,
        title: resourceMap.get(tab.resourceId)!.title,
        type: resourceMap.get(tab.resourceId)!.type,
      }))
    const nextActive = validTabs.some((tab) => tab.resourceId === stored.activeId)
      ? stored.activeId
      : validTabs[0]?.resourceId ?? null
    setTabs(validTabs)
          ...
          return (
            <div key={tab.resourceId} className={`flex shrink-0 items-center border-r border-[var(--border)] ${active ? 'bg-[var(--background)]' : 'bg-[var(--surface)]'}`}>
              ...
              <button
                ...
                onClick={() => void close(tab.resourceId)}
                className="mr-2 inline-flex h-7 w-7 items-center justify-center text-base text-[var(--muted-foreground)] hover:bg-[var(--surface-secondary)] hover:text-[var(--foreground)]"
              >
                ×
              </button>
            </div>
      ...
      {activeId ? (
        <div id={`workspace-panel-${activeId}`} role="tabpanel" aria-label={resourceMap.get(activeId)?.title ?? 'Active resource'} className="mt-6">
          <p className="text-sm text-[var(--muted-foreground)]">
            Active resource: <span className="font-medium text-[var(--foreground)]">{resourceMap.get(activeId)?.title}</span>
          </p>
        </div>
      ) : (
